...
def ddstore_scrape():
    ...
    for key, url in ddstore.items():
        if os.path.isfile(f"./scraped_data/ddstore_{today}/ddstore_{key}.json"):
            continue
        ...
        while True:
            ...
            for product in product_tags:
                ...
                product_object = models.Product(
                    store_name=stores.__DDSTORE__, product_url=product_details_url, product_type=get_type(key))
                print(type(product_object.name))
                product_object.name = product_soup.select_one(
                    'h1.page-title > span.base').text.strip()
                print("Name: " + product_object.name)
                product_object.original_id = product_soup.select_one(
                    'div.product-info-sku > span').text.split('КОД:')[1].strip()
                print("OGID: " + product_object.original_id)
                product_object.image_url = product.select_one(
                    'img.product-image-photo')['src'].strip()
                ...
                additional_description = product_soup.select(
                    'label.for-tooltip')
                ...
                for ad in additional_description:
                    product_object.description = product_object.description + \
                        f"{ad.text}\n"
                print("Done add desc")
                product_object.brand = product_soup.select_one(
                    'div.featured-attributes-rows > div:first-child > label').text.split('Бренд :')[1].strip()
                print("Brand: " + product_object.brand)
                if product.select_one('span.price') is None:
                    continue
                product_object.price_mkd = ''.join(
                    filter(str.isdigit, product.select_one('span.price').text))
                print("Price: " + product_object.price_mkd)

                product_object.availability_list = "DDStore LTD (ДДСтор);"

                availability_image_src = product.select_one(
                    'div.product-info-stock-label > span.listing-info-row > img')['src']
                print("Availability image src: " + availability_image_src)
                if 'not-available' in availability_image_src:
                    product_object.is_available = False
                elif 'available' in availability_image_src:
                    product_object.is_available = True
                else:
                    product_object.is_available = None

                products.append(product_object.toJson())

                print(
                    f"Sleeping 2.5 second after adding new product #{len(products)}")
                # time.sleep(2.5)
