        ...
        while True:
            ...
            for product in product_tags:
                ...
                if(len(product_soup.select('div.product-desc > a')) > 0):
                    product_object.brand = product_soup.select('div.product-desc > a')[0].get_text(strip=True)
                else:
                ...
                store_tags = product_soup.select('div.span4.clearfix.pad5 > ul > li')
                ...
                print(f"Sleeping 20 second after adding new product #{len(products)}")
                time.sleep(20)
